...
class pythonNode2(Node, NodeBase):
    def __init__(self, name, graph):
        super(pythonNode2, self).__init__(name, graph)
        self.menu = QMenu()
    ...
    def postCreate(self, jsonTemplate):
        ...
        #TODO jpd fix this shit
        #https: // stackoverflow.com / questions / 20540141 / why - does - types - methodtype - complain - too - many - arguments

        self.compute = MethodType(Node.compute, self)
    ...
    def export(self):
        ...
        inputs = []
        portString = ""
        for obj in list(self.inputs.values()):
            if obj.dataType == DataTypes.Exec:
                comp = "self.compute"
            else:
                comp = "None"
            portString += """\n        self.{0} = self.addInputPin("{0}", {1},{2},hideLabel={3})""".format(obj.name,
                                                                                                           str(
                                                                                                               obj.dataType),
                                                                                                           comp,
                                                                                                           obj.bLabelHidden)

        for obj in list(self.outputs.values()):
            comp = "None"
            portString += """\n        self.{0} = self.addOutputPin("{0}", {1},hideLabel={2})""".format(obj.name, str(
                obj.dataType), obj.bLabelHidden)
    ...
    def _implementPlugin(self, name, ports, computeCode):
        ...
        if not pth == '':
            ...
            NodeTemplate = """
from PyFlow.Core.AGraphCommon import DataTypes, pinAffects
