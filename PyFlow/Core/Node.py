...
class Node(QGraphicsItem, NodeBase):
    ...
    def call(self, name):
        if pinName in [p.name for p in list(self.outputs.values()) if p.dataType is DataTypes.Exec]:
            p = self.getPinByName(pinName)
            return p.call()

    def getData(self, pinName):
        if pinName in [p.name for p in list(self.inputs.values())]:
            p = self.getPinByName(pinName, PinSelectionGroup.Inputs)
            return p.getData()

    def setData(self, pinName, data):
        if pinName in [p.name for p in list(self.outputs.values())]:
    ...
    def initializeFromFunction(foo, graph):
        ...
        for i in list(inst.inputs.values()):
    ...
    def onUpdatePropertyView(self, formLayout):
        ...
        # inputs
        if len([i for i in list(self.inputs.values())]) != 0:
            sep_inputs = QLabel()
        ...
        # outputs
        if len([i for i in list(self.outputs.values())]) != 0:
            sep_outputs = QLabel()
