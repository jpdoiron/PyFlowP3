...
class NodeBoxTreeWidget(QTreeWidget):
    ...
    def refresh(self, dataType=None, pattern='', pinType=None):
        ...
        if dataType is not None:
            for categoryItem in list(self.categoryPaths.values()):
    ...
    def mousePressEvent(self, event):
        ...
        if pressed_text in list(self.categoryPaths.keys()):
            event.ignore()
            return
...
class GraphWidget(QGraphicsView, Graph):
    ...
    def shoutDown(self):
        for ed in list(self.codeEditors.values()):
            ed.deleteLater()
    ...
    def getGraphSaveData(self):
        ...
        # save edges
        data[self.name]['edges'] = [e.serialize() for e in list(self.edges.values())]
        # variables
        data[self.name]['variables'] = [v.serialize() for v in list(self.vars.values())]
        return data
    ...
    def mouseMoveEvent(self, event):
        ...
        if self._draw_real_time_line:
            ...
            if self.real_time_line not in list(self.scene().items()):
    ...
    def mouseReleaseEvent(self, event):
        ...
        if self._draw_real_time_line:
            self._draw_real_time_line = False
            if self.real_time_line in list(self.scene().items()):
    ...
    def plot(self):
        ...
        if self.parent:
            ...
            for k, v in self.vars.items():
                ...
                print(msg)
            print('Pins\n-----------------')
            for pinUid, pin in self.pins.items():
                msg = '{0} - {1}'.format(pinUid, pin.name, str(pin.uid))
                print(msg)
            print('Edges\n-----------------')
            for edgeUid, edge in self.edges.items():
                print((edgeUid, edge))
