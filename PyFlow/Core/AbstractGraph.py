        ...
        if self.direction == PinDirection.Input:
            if self.dirty or self.parent().bCallable:
                ...
                if not out == []:
                    ...
                    for layer in reversed(sorted([i for i in list(compute_order.keys())])):
...
class NodeBase(INode):
    ...
    def getUniqPinName(self, name):
        pinNames = [i.name for i in list(self.inputs.values()) + list(self.outputs.values())] + dir(self) + keyword.kwlist
...
class Graph(object):
    ...
    def getUniqVarName(self, name):
        names = [v.name for v in list(self.vars.values())]
    ...
    def getUniqNodeName(self, name):
        nodes_names = [n.name for n in list(self.nodes.values())]
        ...
        for n in self.getNodes():
            ...
            for inp in list(n.inputs.values()):
                ...
                for e in inp.edge_list:
                    print('\t|---', e.__str__())
            for out in list(n.outputs.values()):
